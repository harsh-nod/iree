...
static llvm::Optional<vector::CombiningKind> getKindForOp(Operation *scanOp) {
...
struct VectorizeScanPattern : public OpRewritePattern<ScanOp> {
  ...
  LogicalResult matchAndRewrite(ScanOp op,
    ...
    VectorType ivType =
        VectorType::get(accumulatorShape, accumulatorType.getElementType());
    VectorType readType =
        VectorType::get(inputShape, inputType.getElementType());
    ...
    Value readValue = rewriter.create<vector::TransferReadOp>(
        loc, readType, op.input(), indices);
    bvm.map(op.region().getArgument(0), readValue);
    SmallVector<Value> accIndices = indices;
    accIndices.pop_back();
    Value accValue = rewriter.create<vector::TransferReadOp>(
        loc, ivType, op.accumulator(), accIndices);

    llvm::Optional<vector::CombiningKind> maybeKind;
    maybeKind = getKindForOp(&(op.region().front().front()));
    if (!maybeKind) return failure();
    int64_t reductionDim = op.dimension();
    auto scanOp = rewriter.create<vector::ScanOp>(
        loc, readType, ivType, *maybeKind, readValue, accValue, reductionDim,
        op.inclusive());
    auto writeScanResultOp = rewriter.create<vector::TransferWriteOp>(
        loc, scanOp.dest(), op.output(), indices);
    auto writeReductionResultOp = rewriter.create<vector::TransferWriteOp>(
        loc, scanOp.accumulated_value(), op.accumulator(), accIndices);
