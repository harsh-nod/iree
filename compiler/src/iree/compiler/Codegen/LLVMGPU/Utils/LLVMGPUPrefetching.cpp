...
public:
  ...
  void getReachable(Operation *op, DenseSet<Operation *> &reachable, bool noTransferReads = false) {
    if (!op)
      return;
    ...
    if (noTransferReads) {
      if (isa<vector::TransferReadOp>(op)) return;
    }
  ...
  void initializeStages() {
    ...
    for (auto &op : readStage) { op->dump(); }
    llvm::dbgs() << "--- Write Stage ---\n";
    for (auto &op : writeStage) { op->dump(); }
    llvm::dbgs() << "--- Compute Stage ---\n";
    for (auto &op : computeStage) { op->dump(); }
    });
  ...
  SmallVector<Value> emitRead(IRMapping &mapping, RewriterBase &rewriter, Value iv) {
  ...
  SmallVector<Value> emitCompute(IRMapping &mapping, RewriterBase &rewriter, Value iv) {
    mapping.map(forOp.getInductionVar(), iv);
    SmallVector<Value> results;
  ...
  void updateYield(IRMapping &mapping, SmallVector<Value> &readValues, RewriterBase &rewriter) {
    for (auto op : computeStage) {
      if (auto yield = dyn_cast<scf::YieldOp>(op)) {
        ...
        operands.append(readValues.begin(), readValues.end());
  ...
  std::tuple<SmallVector<Value>, SmallVector<Value>> emitPrologue(RewriterBase &rewriter) {
    Value zero = rewriter.create<arith::ConstantIndexOp>(forOp.getLoc(), lb);
    Value one = rewriter.create<arith::ConstantIndexOp>(forOp.getLoc(), lb + step);
  ...
  static void createWriteMappings(RewriterBase &rewriter, SmallVector<Value> &srcValues,
                                  SmallVector<Value> &targetValues, IRMapping &mapping) {
    for (int i = 0; i < srcValues.size(); i++) {
      mapping.map(srcValues[i], targetValues[i]);
    }
  }

  scf::ForOp createKernelLoop(RewriterBase &rewriter, SmallVector<Value> &newIterArgs, SmallVector<Value> &writeArgs) {
    int64_t newUpperBound = singleStage ? ub - step : ub - 2 * step;
    auto newUb =
        rewriter.create<arith::ConstantIndexOp>(forOp.getLoc(), newUpperBound);

    // Keep original iter args and then add some for what's being loaded to registers
    SmallVector<Value> iterArgs(forOp.getInitArgs().begin(), forOp.getInitArgs().end());
    if (!newIterArgs.empty())
      iterArgs.append(newIterArgs.begin(), newIterArgs.end());

    Value newStep = singleStage ?  forOp.getStep() :
        rewriter.create<arith::AddIOp>(forOp.getLoc(), forOp.getStep(), forOp.getStep());
    auto newForOp = rewriter.create<scf::ForOp>(
        forOp.getLoc(), forOp.getLowerBound(), newUb, newStep,
        iterArgs);
  ...
  void createKernel(scf::ForOp newForOp, RewriterBase &rewriter) {
    ...
    Value increment = rewriter.create<arith::ConstantIndexOp>(forOp.getLoc(), step);
    Value iPlusOne = rewriter.create<arith::AddIOp>(forOp.getLoc(), i, increment);
    Value iPlusTwo = rewriter.create<arith::AddIOp>(forOp.getLoc(), iPlusOne, increment);
    Value iPlusThree = rewriter.create<arith::AddIOp>(forOp.getLoc(), iPlusTwo, increment);
  ...
  Value emitEpilogue(scf::ForOp newForOp, RewriterBase &rewriter, SmallVector<Value> &writeArgs) {
    Location loc = forOp.getLoc();
    Value nMinusTwo = rewriter.create<arith::ConstantIndexOp>(loc, ub - 2*step);
    Value nMinusOne = rewriter.create<arith::ConstantIndexOp>(loc, ub - 1*step);
...
FailureOr<scf::ForOp> applyPrefetching(scf::ForOp forOp) {
  ...
  scf::ForOp newForOp  = prefetcher.createKernelLoop(rewriter, iterArgs, writeArgs);
  prefetcher.createKernel(newForOp, rewriter);

  rewriter.setInsertionPointAfter(newForOp);
  Value result = prefetcher.emitEpilogue(newForOp, rewriter, writeArgs);
