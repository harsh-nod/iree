...
static Value warpScan(OpBuilder& b, Location loc, Value val,
                      vector::CombiningKind kind, bool isInteger,
                      Type elemType) {
  std::array<Type, 2> shuffleType = {val.getType(), b.getI1Type()};
  Value activeWidth = b.create<arith::ConstantIntOp>(loc, 32, b.getI32Type());
  Value value = val;
  Value zero =
      b.create<arith::ConstantOp>(loc, elemType, b.getZeroAttr(elemType));
  for (int i = 1; i < kWarpSize; i <<= 1) {
    ...
    auto shuffleOp = b.create<gpu::ShuffleOp>(
        loc, shuffleType, value, offset, activeWidth, gpu::ShuffleMode::UP);
    auto selectOp = b.create<SelectOp>(loc, shuffleOp->getResult(1),
                                       shuffleOp->getResult(0), zero);
    value = createElementwiseOp(b, loc, value, selectOp->getResult(0), kind,
                                isInteger);
  }
  return value;
}
...
static Value broadcastToAllLanes(OpBuilder& b, Location loc, Value val) {
  ...
  return b
      .create<gpu::ShuffleOp>(loc, shuffleType, val, zero, activeWidth,
                              gpu::ShuffleMode::IDX)
      .getResult(0);
}

struct ConvertScanToGPU final : public OpRewritePattern<vector::ScanOp> {
  using OpRewritePattern<vector::ScanOp>::OpRewritePattern;

  LogicalResult matchAndRewrite(vector::ScanOp op,
                                PatternRewriter& rewriter) const override {
    if (op.getSourceType().getNumElements() != kWarpSize) return failure();
    ...
    auto vecType =
        VectorType::get(SmallVector<int64_t>(op.getSourceType().getRank(), 1),
    ...
    if (initialVecType.getRank() > 0) {
      ...
      distributedAccVal = rewriter.create<vector::ExtractElementOp>(
          op.getLoc(), op.initial_value());
    }
    ...
    v = createElementwiseOp(rewriter, op.getLoc(), distributedAccVal, v,
                            op.kind(), isInteger);

    Value broadcastVec = rewriter.create<vector::BroadcastOp>(
        op.getLoc(), VectorType::get({1}, op.getSourceType().getElementType()),
        v);
    Value v1 = rewriter.create<vector::InsertMapOp>(op.getLoc(), broadcastVec,
                                                    op.dest(), laneId);

    Value v2 = broadcastToAllLanes(rewriter, op.getLoc(), v);
    if (initialVecType.getRank() == 0) {
      v2 = rewriter.create<vector::BroadcastOp>(
          op.getLoc(), VectorType::get({}, op.getSourceType().getElementType()),
          v2);
    }
